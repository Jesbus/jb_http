...
string compileConfigScript(string confFile, vector<string>* strings, vector<string>* regexs, vector<string>* inputs)
{
	...
	vector<unsigned char>* dataIdStack = new vector<unsigned char>();
	...
	for(int i=0; i<confFile.length(); i++)
	{
		if (beginningOfLine)
		{
			if (inBrackets!=0)
			{
				if (inBrackets==1) printf("Notice: One bracket wasn't closed on line %i.\n", line-1);
				else if (inBrackets>1) printf("Notice: %i brackets were left unclosed on line %i.\n", inBrackets, line-1);
				inBrackets = 0;
			}
		...
		else if ((!inRegex)&&(!inString)&&(confFile[i]==')')) // Exit bracketed part
		{
			...
			for (int j=dataStack->size()-1;j>=0;j--)
			{
				...
				if (j==0)
				{
					printf("%s%i%s%i%s\n", "Notice: On line ", line, ", col ", (i+1), ", extra closing bracket ignored.");
		...
		else if (inString&&(confFile[i]=='"')) // Exit string
		{
			...
			strings->push_back(s);//
			if (inCommandStatement && inBrackets==0)
			{
			}
			else
			{
				...
				else
				{
					...
					if (osSize>=3)
					{
						...
						if ((o1==0x4C) && (o2==0x4D) && (o3==0x4E)) // replace
						{
							if (debug)
							{
								printf("\ninreplace dataStack->size()=%i", dataStack->size());
							
								for (int k=0;k<dataStack->size();k++)
								{
									printf("\ndataStack->at(%i)=%s", k, bin2hex(dataStack->at(k)).c_str());
								}
							}
							...
							if (debug) printf("\nyup replace. operatorStack->size()=%i dataStack->size()=%i", operatorStack->size(), dataStack->size());
							operatorStack->pop_back();
							operatorStack->pop_back();
							operatorStack->pop_back();
							
							if (debug) printf("\nyup replace. operatorStack->size()=%i", operatorStack->size());
							///////////////////////////////////////
							if (d1==0x20) // String
							{
							...
							else printf("\n[Error] d2=unknown data type");
							
							///////////////////////////////////////
							if (d3==0x20) // String
							{
								//byteCode << (unsigned char)d3;
								//bcStringCount++;
								//byteCode << (unsigned char)bcStringCount;
							}
							else if (d3==0x21) // Regex
							{
								//byteCode << (unsigned char)d3;
								//bcRegexCount++;
								//byteCode << (unsigned char)bcRegexCount;
							}
							else if (d3==0x22 || d3==0x25 || d3==0x27 || d3==0x28 || d3==0x29) // Input
							{
								//byteCode << (unsigned char)d3;
								//bcInputCount++;
								//byteCode << (unsigned char)bcInputCount;
							}
							else if (d3==0x23) // placeholder
							{
								if (debug) printf("\nd3=placeholder");
							}
							else printf("\n[Error] d3=unknown data type");
							...
							if (debug)
							{
								...
								string bcSoFar = byteCode.str();
								for (int k=0;k<bcSoFar.length();k++)
								{
									printf("\nbyteCode[%i]=%s", k, bin2hex(bcSoFar[k]).c_str());
								}
							}
						}
					}
