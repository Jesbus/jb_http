	...
	if (responseCode==200 || responseCode==404)
	{
		...
		for (int i=0;i<strlen(confScript);i++)
		{
			...
			case 0x20:{ // string
				...
				i++;
				if (debug) printf("\n#####confScript[%i]=%s=strid", i, bin2hex(confScript[i]).c_str());
			...
			case 0x49:{ // concat
				...
				if (d1==0x20 && d2==0x20) // two strings
				{
					...
					{
						allStrings += datas->at(k)->c_str();
						allStrings += '-';
					}
					if (debug) printf("\ndatas->size()=%i allStrings=%s", datas->size(), allStrings.c_str());*/
			...
			{
				if (datas->back()==&boolFalse) // False -> True
				{
					datas->pop_back();
					datas->push_back(&boolTrue);
				}
